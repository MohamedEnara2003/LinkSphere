...
import { CommentsStateService } from '../../comments/services/state/comments-state.service';
  ...
  sendFriendRequest(userReceiver : IUser): Observable<FriendRequestResponse> {
    return this.#singleTonApi.create<FriendRequestResponse>(
      `${this.routeName}/friend-request/${userReceiver._id}`).pipe(
        ...
        this.#commentsState.updatePostLikerFlag(receiver._id, 'requestSent');
...
acceptFriendRequest(requestId: string, sender: Author): Observable<void> {
  return this.#singleTonApi
    ...
    .pipe(
      tap(() => {
        ...
        this.#commentsState.updatePostLikerFlag(sender._id, 'notFriend');
      }),
    );
}
  ...
  this.#commentsState.updatePostLikerFlag(receiverId, 'notFriend');
  }
  ...
  return this.#singleTonApi
    ...
    .pipe(
      tap(() => {
        ...
        this.#commentsState.updatePostLikerFlag(friendId, 'notFriend');
      }),
    );
}
