...
<app-user-profile-header [isMyProfile]="isMyProfile()"/>
</article>
...
<section class="w-full md:h-100 flex flex-col gap-5 md:sticky md:top-40">

@if(isMyProfile()){
<button [routerLink]="['/public/profile', userProfileService.userProfile()?._id, 'update']"
type="button" 
class="ngBtn ">
...
isMyProfile = computed<boolean>(() => 
this.userProfileService.user()?._id === this.userId()
)
