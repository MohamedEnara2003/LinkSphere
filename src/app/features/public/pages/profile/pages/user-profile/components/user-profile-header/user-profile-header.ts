...
unFriend() : void {
  const  {_id} = this.userProfileService.userProfile()!;
  if(!_id) return ;
  this.userProfileService.unFriend(_id).subscribe();
}
...
stateFriend() : void {
  ...
  const isFriend = (this.userProfileService.user()?.friends || []).some((f) => {
  return f._id === this.userProfileService.userProfile()?._id
  })
